...
1. **`run_aparavi_report`** - Use for standard business questions covered by the 20 predefined reports
   - Example: "Show me duplicate files" or "Analyze storage by data source"
   - Use `report_name="list"` to see all available reports

2. **`generate_aql_query`** - Use for custom analysis NOT covered by predefined reports
...
3. **`validate_aql_query`** - Use to check AQL syntax before execution
...
4. **`execute_custom_aql_query`** - Use to run validated custom AQL queries
   - Combines validation + execution in one step
   - Returns raw JSON results for analysis

5. **`health_check`** - Use to verify system status and API connectivity
6. **`server_info`** - Use to view server configuration and capabilities

**Recommended Workflow for Custom Analysis:**
```
Custom Question → generate_aql_query → validate_aql_query → execute_custom_aql_query
```
